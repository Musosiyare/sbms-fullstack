...
import { exportConductReportPdf } from "../utils/pdf";
 ...
 *
 * This used to offer a native window.print() with a print stylesheet that
 * hid everything but the report and pinned it to one A4 page. That hit a
 * Chromium print-pagination bug (a block right at the page-height boundary
 * gets retried across 2-3 pages, each printing the full content again — so
 * "print one report" came out as 2-3 duplicate copies). CSS-only fixes for
 * that bug proved fragile (they'd hold until the content's height shifted
 * again — a longer incident list, an extra header line, etc. — and the
 * duplication would come back). Generating the same one-page layout as a
 * real PDF via jsPDF (same code path as the whole-class download in
 * ClassConductReportModal, which never had this problem) sidesteps the
 * browser's print pagination entirely, so it's used here too.
 */
export default function ConductReportModal({ studentId, termId, academicYearId, onClose }) {
  ...
  function handleDownload() {
    ...
    setDownloading(true);
    try {
      const fileBase = `${slugify(report.student.firstName)}-${slugify(report.student.lastName)}-${slugify(report.term.name)}`;
      exportConductReportPdf([report], `${fileBase}-conduct-report.pdf`);
    } catch (err) {
